(function(){
 ...
 window.activeTrip=window.db.trips[0]?.id||'trip-main';window.activePlan=window.db.trips[0]?.plans?.[0]?.id||'A';window.activeDay=0;
 ...
 window.createTripFromForm=function(){
   const name=document.getElementById('ntName')?.value.trim(),city=document.getElementById('ntCity')?.value.trim(),start=document.getElementById('ntStart')?.value,end=document.getElementById('ntEnd')?.value,alternate=!!document.getElementById('ntAlternate')?.checked;
   if(!name||!start||!end)return window.toast?.('请填写行程名称和日期');
   if(end<start)return window.toast?.('结束日期不能早于开始日期');
   const days=[];let d=new Date(start+'T00:00:00'),last=new Date(end+'T00:00:00'),i=0;
   while(d<=last&&i<31){const iso=d.toISOString().slice(0,10);days.push({id:uid(),label:'DAY '+i,date:iso,title:'待安排',items:[]});d.setDate(d.getDate()+1);i++;}
   const makeDays=()=>JSON.parse(JSON.stringify(days)).map(x=>({...x,id:uid(),items:(x.items||[]).map(y=>({...y,id:uid()}))}));
   const plans=[{id:'A',name:'方案 A',days:makeDays()}];
   if(alternate)plans.push({id:'B',name:'方案 B',days:makeDays()});
   const t={id:uid(),name,city:city||'未设置目的地',start,end,people:1,hasAlternateRoutes:alternate,plans};
   window.db.trips.push(t);window.activeTrip=t.id;window.activePlan='A';window.activeDay=0;window.save();window.closeModal?.();window.go?.('trips');window.renderTrips?.();window.toast?.(alternate?'行程创建成功，已生成方案 A / B':'行程创建成功');if(window._createMode==='ai')setTimeout(()=>window.go?.('ai'),120);
 };
 ...
 window.deleteItem=function(i){const d=window.currentPlan()?.days?.[window.activeDay];if(!d?.items?.[i])return;if(confirm('删除这条日程？')){d.items.splice(i,1);window.save();window.renderTripDetail?.();}};
