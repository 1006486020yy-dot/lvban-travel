...
(function(){
  ...
  function allTripCities(t){const out=[];const push=v=>{if(Array.isArray(v))v.forEach(push);else if(typeof v==='string')v.split(/[→,，、/|\\s]+/).map(text).filter(Boolean).forEach(x=>out.push(x))};['city','cities','route','routeCities','destinations','destination'].forEach(k=>push(t?.[k]));arr(t?.plans).forEach(p=>{['city','cities','route','routeCities','destinations','destination'].forEach(k=>push(p?.[k]));arr(p?.days).forEach(d=>push(cityOfDay(d)))});return [...new Set(out)]}
  function sameCity(a,b){const x=text(a),y=text(b);return !!x&&!!y&&(x===y||x.includes(y)||y.includes(x))}
  function tripHasCity(t,city){if(!city)return true;return allTripCities(t).some(c=>sameCity(c,city))}
  function plansForCity(t,city){return arr(t?.plans).filter(p=>{const direct=allTripCities({...t,plans:[p]});return !city||direct.some(c=>sameCity(c,city))})}
  function daysForCity(p,city){const days=arr(p?.days).map((d,i)=>({d,i}));const cityDays=city?days.filter(x=>sameCity(cityOfDay(x.d),city)):days;return (cityDays.length?cityDays:days).sort((a,b)=>dayOrder(a.d,a.i)-dayOrder(b.d,b.i))}
  function injectStyle(){if(document.getElementById('lvbanUnifiedAddFlowStyle'))return;const s=document.createElement('style');s.id='lvbanUnifiedAddFlowStyle';s.textContent=`
  ...
  function addFromCatalogUnified(item){
    ...
    function refreshPlans(preferredPlanId){const t=candidates.find(x=>idOf(x)===String(tripSel.value))||candidates[0],plans=plansForCity(t,city);planSel.innerHTML=plans.map((p,i)=>`<option value="${esc(idOf(p))}">${esc(p.name||`方案 ${i+1}`)}</option>`).join('');if(preferredPlanId&&plans.some(p=>idOf(p)===preferredPlanId))planSel.value=preferredPlanId;refreshDays()}
    function refreshDays(preferredDayIndex){
      const t=candidates.find(x=>idOf(x)===String(tripSel.value))||candidates[0],plans=plansForCity(t,city),p=plans.find(x=>idOf(x)===String(planSel.value))||plans[0];
      if(!p){daySel.innerHTML='';hint.textContent='该大行程没有包含当前城市的方案。';return}
      const list=daysForCity(p,city);daySel.innerHTML=list.map(x=>{const d=x.d,count=dayItems(d).length;return `<option value="${x.i}">${esc(dayLabel(d,x.i))} · ${esc(dayDate(d)||'日期待定')}${count?` · ${count}项`:''}</option>`}).join('');
      let target=list[list.length-1];
      if(preferredDayIndex!=null&&list.some(x=>x.i===preferredDayIndex))target=list.find(x=>x.i===preferredDayIndex);else{const withItems=list.filter(x=>dayItems(x.d).length);if(withItems.length)target=withItems[withItems.length-1]}
      if(target)daySel.value=String(target.i);const selected=list.find(x=>String(x.i)===String(daySel.value));if(selected){const count=dayItems(selected.d).length;hint.textContent=count?`将作为“第 ${count+1} 站”添加到 ${dayLabel(selected.d,selected.i)}。`:`该日暂无行程，将作为“第一站”添加。`}
    }
    tripSel.onchange=()=>refreshPlans();planSel.onchange=()=>refreshDays();daySel.onchange=()=>refreshDays(Number(daySel.value));
    mask.querySelector('.lv-add-unified-confirm').onclick=()=>{const t=candidates.find(x=>idOf(x)===String(tripSel.value)),plans=plansForCity(t,city),p=plans.find(x=>idOf(x)===String(planSel.value))||plans[0],i=Number(daySel.value),d=p?.days?.[i];if(!t||!p||!d){alert('请选择有效的大行程、方案和日期');return}window.activeTrip=t.id;window.activePlan=p.id;window._lv2Day=i;window._lv2City=cityOfDay(d)||city;window._lvbanCatalogPending=item;mask.remove();if(typeof window.__lvbanOpenCreateDay==='function')window.__lvbanOpenCreateDay();else alert('每日行程入口暂未加载，请稍后再试')};
    refreshPlans();
  }
  // 覆盖旧版目录入口，确保三个板块完全使用同一套逻辑。
  function install(){window.addFromCatalog=addFromCatalogUnified;window.LVBAN_UNIFIED_ADD_FLOW=addFromCatalogUnified}
  install();
  [0,100,300,800,1500,2500].forEach(ms=>setTimeout(install,ms));
})();
