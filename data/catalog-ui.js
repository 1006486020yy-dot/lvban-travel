...
(function () {
  ...
  function toolbar(target,type){
    ...
    return `<div class="catalog-toolbar" data-catalog-toolbar="${target}"><div class="catalog-search-wrap"><span class="catalog-search-icon">⌕</span><input class="catalog-search" id="${target}Search" value="${esc(q)}" placeholder="搜索${type==='spot'?'景点名称、地址或关键词':type==='food'?'美食店铺、菜品或地址':'酒店名称、地址或关键词'}" oninput="LVBAN_CATALOG.setQuery('${type}',this.value)"><button class="catalog-clear" onclick="LVBAN_CATALOG.setQuery('${type}','')">×</button></div></div><div class="catalog-recommend-row" data-catalog-recommend="${target}"><span class="catalog-recommend-title">推荐城市</span>${RECOMMENDED.map(c=>`<button type="button" class="city-chip" data-city="${esc(c)}" onclick="LVBAN_CATALOG.setCity('${type}','${esc(c)}')">${esc(c)}</button>`).join('')}</div>`;
  }
  ...
  function removeLegacyCityBar(type){
    if(type==='spot'){
      const old=document.getElementById('spotCities');
      if(old){old.innerHTML='';old.style.display='none';}
    }
  }
  function ensureTool(target,type){
    const host=document.getElementById(target);if(!host)return;
    removeLegacyCityBar(type);
    document.querySelectorAll(`[data-catalog-toolbar="${target}"]`).forEach((el,i)=>{if(i>0)el.remove()});
    document.querySelectorAll(`[data-catalog-recommend="${target}"]`).forEach((el,i)=>{if(i>0)el.remove()});
    if(!document.querySelector(`[data-catalog-toolbar="${target}"]`))host.insertAdjacentHTML('beforebegin',toolbar(target,type));
  }
  function render(type){
    const data=window.LVBAN_DATA||{},key=type==='spot'?'spots':type==='food'?'foods':'hotels',target=type==='spot'?'spotList':type==='food'?'foodList':'hotelList',items=Array.isArray(data[key])?data[key]:[];
    ensureTool(target,type);
    const result=filter(items,type),host=document.getElementById(target);if(!host)return;
