...
(function(){
  ...
  window.renderTrips=function(){document.getElementById('tripList').innerHTML=db.trips.map(t=>`<button type="button" class="trip-card ${t.id===activeTrip?'active':''}" onclick="openTripCanvas('${t.id}')"><strong>${safe(t.name)}</strong><small>${safe(t.city||'多城市')}<br>${safe(t.start||'')} → ${safe(t.end||'')}</small></button>`).join('')+`<button type="button" class="trip-card" onclick="openNewTrip()"><strong>＋ 新建行程</strong><small>只填写基础信息，自动生成 DAY</small></button>`;renderTripDetail()};
