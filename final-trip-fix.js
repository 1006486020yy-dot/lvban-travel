...
(function(){
  ...
  function normalize(){
    trips().forEach(t=>{
      ...
      t.plans.forEach(p=>{p.days=Array.isArray(p.days)?p.days:[];p.days.forEach(d=>{d.items=Array.isArray(d.items)?d.items:[];d.items.forEach(x=>{if(typeof x==='object'&&!x.id)x.id=uid()})})});
    });
    save();
  }
