/* 旅伴旅行管家 · 行程详情 V3
   固定结构：
   行程名称
   当前行程涉及的城市（只显示实际城市，不显示“全部城市/景点/美食/交通/酒店”）
   ↓ 点击城市
   该城市对应的日期横排
   ↓ 点击日期
   当天行程详情
...
(function(){
  ...
  function alternateEnabled(t){
    return t?.hasAlternateRoutes===true||t?.hasAlternateRoute===true||t?.alternateRoute===true||t?.useAlternateRoutes===true||t?.hasBackupRoute===true;
  }

  // 只建立“真正属于当前行程”的城市列表。
  // 优先使用 cityDurations；其次使用每天明确记录的 city；最后才从当天项目推断。
  function cityList(t,p){
    ...
    if(Array.isArray(t?.cityDurations)&&t.cityDurations.length){
      t.cityDurations.forEach(x=>add(x?.city));
    }
    if(!out.length){
      (p?.days||[]).forEach(d=>add(d?.city));
    }
    if(!out.length){
      (p?.days||[]).forEach(d=>(d?.items||[]).forEach(x=>add(x?.city)));
    }
    if(!out.length){
      String(t?.city||'').split(/[·,、/|→＞>]+/).forEach(add);
    }
    return out;
  }
  ...
  function dayCity(t,p,i){
    ...
    const ds=Array.isArray(t?.cityDurations)?t.cityDurations:[];
    let n=0;
    for(const x of ds){
      n+=Number(x?.days)||0;
      if(i<n)return String(x?.city||'').trim();
    }
    const itemCities=[...new Set((d?.items||[]).map(x=>String(x?.city||'').trim()).filter(Boolean))];
    if(itemCities.length===1)return itemCities[0];
    return '';
  }

  function daysForCity(t,p,city){
    const result=[];
    (p?.days||[]).forEach((d,i)=>{if(dayCity(t,p,i)===city)result.push(i)});
    return result;
  }
  ...
  function style(){
    ...
    s.textContent=`
      .lv2{padding:5px 2px 105px}
      .lv2-head{display:flex;align-items:center;gap:10px;padding:7px 0 14px}
      .lv2-back,.lv2-more{width:42px;height:42px;border-radius:14px;background:#fff;font-size:27px}
      .lv2-more{font-size:22px}.lv2-title{flex:1}.lv2-title h2{margin:0;font-size:22px}.lv2-title p{margin:4px 0 0;color:var(--muted);font-size:11px}
      ...
      .lv2-event{display:grid;grid-template-columns:58px 1fr;gap:9px;padding:8px 12px 8px 10px}.lv2-time{color:var(--p);font-size:12px;font-weight:900;padding-top:10px;text-align:right}
      ...
      .lv2-address,.lv2-note{font-size:11px;color:var(--muted);line-height:1.55}.lv2-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}.lv2-actions button{padding:8px 10px;border-radius:11px;background:#fff;color:#5d4de5;font-size:11px}
  ...
  function render(){
    style();
    const page=$('#trips'),t=current(),p=plan();
    if(!page||!t||!p)return;
    const cities=cityList(t,p);
    let city=window._lv2City;
    if(!cities.includes(city))city=cities[0]||'';
    window._lv2City=city;
    const indices=daysForCity(t,p,city);
    let dayIndex=Number(window._lv2Day);
    if(!indices.includes(dayIndex))dayIndex=indices[0]??0;
    window._lv2Day=dayIndex;
    ...
    page.innerHTML=`<div class="lv2">
      ...
      <section class="lv2-day"><div class="lv2-day-meta"><div class="date">${esc(d?.date||'')}</div><h3>${esc(d?.title||d?.label||('DAY '+(dayIndex+1)))}</h3><p>${esc(city)} · ${(d?.items||[]).length} 项安排</p></div>
      ${(d?.items||[]).length?(d.items||[]).map((x,k)=>{const addr=x?.address||'';return `<article class="lv2-event"><div class="lv2-time">${esc(x?.time||'')}</div><div class="lv2-card"><span class="lv2-type">${esc(x?.type||'行程')}</span><h3>${esc(x?.name||'未命名')}</h3>${addr?`<div class="lv2-address">📍 ${esc(addr)}</div>`:''}${x?.note?`<div class="lv2-note">${esc(x.note)}</div>`:''}<div class="lv2-actions">${addr?`<button data-copy="${encodeURIComponent(addr)}">复制地址</button><button data-nav-name="${encodeURIComponent(x?.name||'')}" data-nav-address="${encodeURIComponent(addr)}">导航</button>`:''}<button data-edit="${k}">编辑</button></div></div></article>`}).join(''):`<div class="lv2-empty">当天还没有安排</div>`}</section></div>`;
  ...
  // 统一入口：后续任何脚本都调用这个函数进入新版行程结构。
  window._lvbanTripCanvasV2=render;
  window._lvbanTripCanvas=render;
  window.lvTripLayoutV2={render};
