/* 旅伴旅行管家｜我的行程正确层级 v3
   ...
   同一天跨城市时，该日期同时出现在对应城市；不改变现有视觉体系。
*/
(function(){
  'use strict';
  ...
  // 一天可能同时属于两个城市，例如 10/1：平潭上午 + 厦门下午。
  function citiesOfDay(d){
    const text=[d.title||'',...(d.items||[]).flatMap(x=>[x[1]||'',x[2]||'',x[3]||''])].join(' ');
    const found=[];
    if(/福州/.test(text)) found.push('福州');
    if(/平潭/.test(text)) found.push('平潭');
    if(/泉州|石狮/.test(text)) found.push('泉州');
    if(/厦门|鼓浪屿|集美/.test(text)) found.push('厦门');
    return [...new Set(found)];
  }
  function cityList(arr){
    return [...new Set(arr.flatMap(citiesOfDay))];
  }
  function daysForCity(arr,city){
    return arr.filter(d=>citiesOfDay(d).includes(city));
  }
  ...
  function renderDefaultDetail(){
    ...
    list.innerHTML='';
    const arr=schedules[routeState.plan||'A']||[];
    const cities=cityList(arr);
    if(!routeState.city || !cities.includes(routeState.city)) routeState.city=cities[0]||null;
    const cityDays=routeState.city?daysForCity(arr,routeState.city):[];
    if(!routeState.date || !cityDays.some(x=>x.date===routeState.date)) routeState.date=cityDays[0]?.date||null;
    ...
    detail.innerHTML=detailHeader('十一福建游','福州 · 平潭 · 泉州 · 厦门',`tripFlowBackToList()`)+actionButtons()+
      ...
      `<div class="crumb" style="margin-top:10px">${esc(routeState.city||'')} 的日期</div><div class="days">${cityDays.map(x=>`<button class="day ${x.date===routeState.date?'on':''}" onclick="tripFlowDate('${eAttr(x.date)}')"><b>${esc(x.date.slice(5).replace('-','/'))}</b><small>DAY ${arr.indexOf(x)+1}</small></button>`).join('')}</div>`+
  ...
  function renderCustomDetail(t,i){
    ...
    const city=routeState.city&&cities.includes(routeState.city)?routeState.city:(cities[0]||'');
    routeState.city=city;
    ...
    const date=routeState.date&&dates.includes(routeState.date)?routeState.date:(dates[0]||null);routeState.date=date;
    detail.innerHTML=detailHeader(t.name,`${cities.join(' · ')} · ${t.travelers||1}人`,`tripFlowBackToList()`)+actionButtons()+
      ...
      `<div class="crumb" style="margin-top:14px">选择城市</div><div class="chips">${cities.map(c=>`<button class="chip ${c===city?'on':''}" onclick="tripFlowCustomCity(${i},'${eAttr(c)}')">${esc(c)}</button>`).join('')}</div>`+
      `<div class="crumb" style="margin-top:10px">${esc(city)} 的日期</div><div class="days">${dates.map((x,n)=>`<button class="day ${x===date?'on':''}" onclick="tripFlowCustomDate(${i},'${eAttr(x)}')"><b>${esc(x.slice(5).replace('-','/'))}</b><small>DAY ${n+1}</small></button>`).join('')}</div>`+
  ...
  window.renderTrips=function(){ if(routeState.tripId==='default') return renderDefaultDetail(); if(routeState.tripId&&routeState.tripId.indexOf('custom:')===0){const i=Number(routeState.tripId.split(':')[1]);return customTrips[i]?renderCustomDetail(customTrips[i],i):renderTripList();} return renderTripList(); };
