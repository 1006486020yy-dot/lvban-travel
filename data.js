...
(function(){
  ...
  const mapFood=x=>({id:x.foodId||x.id,foodId:x.foodId||x.id,name:x['店名']||x.name||'',city:x['城市']||x.city||'',address:x['地址']||x.address||'',type:x['类型']||x.type||'',dishes:x['推荐菜']||'',recommended:x['推荐菜']||x['推荐菜']||'',price:x['人均']||'',tags:x['标签']||'',note:x['推荐菜']||'',latlng:x['经纬度']||'',image:x['图片']||''});
  ...
  load('data/catalog-ui.js?v=20260814-8');
  load('trip-card-click-fix.js?v=20260814-4');
  load('trip-card-hard-fix.js?v=20260814-4');
  load('trip-layout-v2.js?v=20260817-2');
  load('trip-delete-fix.js?v=20260814-4');
  load('trip-day-create.js?v=20260814-4');
  load('catalog-add-prefill.js?v=20260814-1');
  load('catalog-add-flow-v2.js?v=20260814-1');
  load('itinerary-v3-lock.js?v=20260814-4');
