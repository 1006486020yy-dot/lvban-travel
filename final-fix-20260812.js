...
(function(){
  ...
  // Capture phase：即使其他脚本拦截 click，关闭按钮仍然优先执行。
  ...
  function showPicker(){
    ...
    ov.innerHTML=`<div class="lv-city-sheet">
      ...
      <div class="lv-city-hint">支持多选，选择完成后点击“确认城市”</div>
    ...
    const render=()=>{
      ...
      const arr=list.filter(c=>!q||c.includes(q)).slice(0,120);
      $('#lvCityList').innerHTML=arr.map(c=>`<button type="button" class="lv-city-option ${picked.includes(c)?'on':''}" data-city="${esc(c)}"><span>${esc(c)}</span><small>${picked.includes(c)?'✓ 已选择':'选择'}</small></button>`).join('')||'<div class="lv-city-empty">没有找到匹配城市</div>';
      $('#lvCityPicked').innerHTML=picked.length?picked.map(c=>`<span>${esc(c)} <button type="button" data-remove="${esc(c)}">×</button></span>`).join(''):'<span class="lv-city-none">暂未选择城市</span>';
    };
  ...
  function updateDestination(){
    ...
    if(field){field.textContent=picked.join(' · ');field.classList.add('has');}
  }
  ...
  function newTrip(){
    ...
    window.openModal('新建行程',`<div class="form lv-newtrip-form">
      <label>行程名称</label>
      <input id="ntName" placeholder="例如：国庆厦门慢旅行">
      <label>目的地</label>
      <button type="button" id="ntDestination" class="lv-destination-field">点击选择城市，可多选 <span>选择城市 ›</span></button>
    ...
    $('#ntDestination').onclick=showPicker;
    $('#manualMode').onclick=()=>chooseMode2('manual');
    $('#aiMode').onclick=()=>chooseMode2('ai');
  ...
  function createTrip(){
    ...
    const days=[];let d=new Date(start+'T00:00:00'),last=new Date(end+'T00:00:00');
    while(d<=last){
      const city=picked[Math.min(days.length,picked.length-1)];
      days.push({id:uid(),label:'DAY '+(days.length+1),date:d.toISOString().slice(0,10),title:picked.length>1?city+' · 待安排':'待安排',city:city,items:[]});
    ...
    window.db=window.db||{trips:[]};window.db.trips=window.db.trips||[];window.db.trips.push(t);
  ...
  const style=document.createElement('style');style.id='lv-final-interaction-style';style.textContent=`
    #lvCityPickerOverlay{position:fixed;inset:0;z-index:20000;background:rgba(23,23,42,.58);display:flex;align-items:flex-end;justify-content:center;padding:0}
    .lv-city-sheet{width:min(720px,100%);max-height:88vh;overflow:hidden;background:#f7f8fc;border-radius:28px 28px 0 0;padding:20px;box-shadow:0 -12px 50px rgba(0,0,0,.18);display:flex;flex-direction:column;gap:10px}
    ...
    .lv-city-picked{display:flex;gap:7px;flex-wrap:wrap;min-height:32px}.lv-city-picked>span{background:#efedff;color:#5d4de5;padding:7px 10px;border-radius:11px;font-size:12px}.lv-city-picked button{background:none;color:#5d4de5;padding:0 0 0 5px}
    ...
    .lv-destination-field{width:100%;padding:13px 14px;border:1px solid #e9e7f2;border-radius:14px;background:#fff;text-align:left;color:#777;display:flex;justify-content:space-between;align-items:center}.lv-destination-field span{color:#5d4de5;font-weight:800}.lv-destination-field.has{color:#333}.lv-newtrip-form .mode{text-align:left}.lv-newtrip-form .mode small{display:block;color:#999;font-size:10px;margin-top:3px}.lv-newtrip-form .mode.on small{color:#777}
