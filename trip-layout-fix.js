/* 旅伴旅行管家 · 行程详情布局修复
   1. 未勾选备选路线：隐藏方案 A / B
   2. 只显示本次行程选择的城市，不显示“全部城市/景点/美食/交通/酒店”混合筛选
   3. 点击城市后，下一行显示该城市对应日期，再下面显示当天行程细节
*/
(function(){
  const esc=v=>String(v??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));
  const css=`
  .lv-trip-city-tabs{display:flex;gap:10px;overflow-x:auto;padding:4px 0 10px;scrollbar-width:none}.lv-trip-city-tabs::-webkit-scrollbar{display:none}
  .lv-trip-city-tab,.lv-trip-date-tab{flex:0 0 auto;border:1px solid var(--line);background:#fff;color:#555;border-radius:14px;cursor:pointer;font-weight:800}
  .lv-trip-city-tab{padding:11px 20px;font-size:14px}.lv-trip-city-tab.on,.lv-trip-date-tab.on{background:#6958f5;color:#fff;border-color:#6958f5;box-shadow:0 7px 18px rgba(105,88,245,.18)}
  ...
  function dayCityMap(t,p){
    ...
    const durations=Array.isArray(t?.cityDurations)?t.cityDurations.filter(x=>x&&x.city&&Number(x.days)>0):[];
    if(durations.length){let idx=0;durations.forEach(x=>{for(let k=0;k<Number(x.days)&&idx<days.length;k++,idx++)out[idx]=x.city})}
    days.forEach((d,i)=>{if(d&&d.city)out[i]=d.city});
    if(!out.filter(Boolean).length){const cs=selectedCities(t);if(cs.length){for(let i=0;i<days.length;i++)out[i]=cs[Math.min(cs.length-1,Math.floor(i*cs.length/days.length))]}}
    return out;
  }
  function render(){
    ...
    const dateTabs=cityDayIndexes.map(i=>{const x=days[i];return `<button type="button" class="lv-trip-date-tab ${i===window.activeDay?'on':''}" data-day="${i}"><b>${esc(x.label||('DAY '+(i+1)))}</b><small>${esc(x.date||'')}</small></button>`}).join('');
    const scheme=`<div class="scheme-head"><div><div class="crumb">${esc(t.name||'行程')}</div><div class="scheme-name">${esc(showPlan?(p.name||'方案 A'):'行程详情')}</div><div class="sub">${esc(activeCity||t.city||'')} · ${t.people||1}人</div></div><div class="actions"><button class="mini" onclick="openPlanEditor()">方案管理</button><button class="mini primary" onclick="openDayEditor()">＋ 添加日程</button></div></div>`;
    const planTabs=showPlan?`<div class="tabs" style="margin-top:10px">${plans.map(x=>`<button type="button" class="tab ${x.id===window.activePlan?'on':''}" data-plan="${esc(x.id)}">${esc(x.name||('方案 '+x.id))}</button>`).join('')}</div>`:'';
