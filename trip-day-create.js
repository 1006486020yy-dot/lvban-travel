...
(function(){
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='lv-create-style';s.textContent=`.lv-create-mask{position:fixed;inset:0;z-index:100;background:#17172a66;display:flex;align-items:flex-end}.lv-create-sheet{width:100%;max-height:90vh;overflow:auto;background:#f7f8fc;border-radius:28px 28px 0 0;padding:20px}.lv-create-grid{display:grid;gap:9px}.lv-create-grid label{font-size:12px;color:#77788b}.lv-create-grid input,.lv-create-grid textarea,.lv-create-grid select{width:100%;padding:12px;border:1px solid #e8e6f4;border-radius:14px;background:#fff;outline:0;box-sizing:border-box}.lv-create-actions{display:flex;gap:8px;margin-top:12px}.lv-create-actions button{flex:1;padding:12px;border-radius:14px;background:#fff;color:#5d4de5;font-weight:800;border:1px solid #e8e6f4}.lv-create-actions .primary{background:#6958f5;color:#fff;border:0}`;document.head.appendChild(s);
  }
  function openForm(){
  ...
  window.__lvbanOpenCreateDay=openForm;
  window.openCreateDayForm=openForm;
  // 兼容旧的渲染器：如果它没有内置按钮，再补一个；V3 已经自带按钮时不会重复。
